...
def download_video(self, fpath, video_id):
    ...
    print('Fetching video metadata from:', meta_url)
    ...
    try:
        ...
        print('Response status code:', meta_res.status_code)

        if meta_res.status_code != 200:
            print('Failed to fetch video meta. Response content:')
            print(meta_res.text)  # Print the response content for further debugging
            raise Exception('Failed to fetch video meta')

        video_data = meta_res.json()
        print('Video metadata fetched successfully.')

        # Continue with the rest of your code to process the video data and download the video

    except Exception as e:
        print('Exception occurred during video metadata fetch:', str(e))
        raise e
